...
class Hello extends CI_Controller
{
	function index()
	{
		...
		//$tag=get_meta_tags(getcwd()."\Rihanna-Stay.mp3");
		echo getcwd()."<br>";
		//print_r($tag);
		echo substr(file_get_contents(getcwd()."\Demi_Lovato-Heart_Attack.mp3"),0,1024);
		echo "<br>";
		$song=$this->tagReader(getcwd()."\Demi_Lovato-Heart_Attack.mp3");
		echo "Title : ".$song['Title']."<br>";
		echo "Album : ".$song['Album']."<br>";
		echo "Artist : ".$song['Author']."<br>";
		echo "Track : ".$song['Track']."<br>";
		echo "Size : ".$song['Size']."<br>";
	}
	
	function tagReader($file){
	$id3v23 = array("TIT2","TALB","TPE1","TRCK","TDRC","TLEN","TSIZ","USLT");
	...
	if($result['Version'] == "4.0" || $result['Version'] == "3.0"){
		for ($i=0;$i<count($id3v23);$i++){
			if (strpos($tag,$id3v23[$i].chr(0))!= FALSE){
				...
				if(substr($tmp,0,4) == "TSIZ") $result['Size'] = substr($tmp,4);
